/* 
   ...
   Uses interrupts.
   Demonstrates the usage of setPosition().
   When the key is pressed, the postion is reset to a fixed value.
...
void setup() {
  ...
  rencoder.setSensitive(false);

  // ***** Prove that we know that no rotation has been performed yet
  int direction = rencoder.getDirection();
  if (direction == rencoder.NOT_MOVED) {
    Serial.println("Current rotation direction is NOT_MOVED");    
  }

  // ****** Start with a set position
  rencoder.setPosition(5);
}
