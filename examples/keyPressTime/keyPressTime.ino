/* 
   ...
   Demonstrates
     ...
     - The reading of the time while the key is pressed
...
int lastPosition;
unsigned long lastTimePressed = 1;
...
void setup() {
  ...
  //*********** Sets the time for the long press key to 500ms
  rencoder.setLongPressTime(500);
...
void loop() {
  ...
  unsigned long currentTimePressed = rencoder.keyPressedTime();
  
  // Could use (rencoder.getSwitchState() == rencoder.SW_ON)  
  if (rencoder.keyPressed()) { 
    if (currentTimePressed > timeThreshold) {  
      Serial.print("Pressed time: ");
      Serial.println(currentTimePressed);
      timeThreshold += 5;
    } else {
  ...
  if (rencoder.getSwitchState() == rencoder.SW_OFF) {
    timeThreshold = 100;      
  }
